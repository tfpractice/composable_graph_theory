...
//     return allMeths;
// };
let basicHooks = (BaseClass) => (sArr) => Object.assign(Array.from(sArr), type_maker(BaseClass), base_type_maker(BaseClass), validator(valid_condition(BaseClass)));
...
let composeMixins = (BaseClass) => (...mixins) => {
    ...
    let composite;
    return (sArr) => thunks.reduce((arr, mixin) => Object.assign([], arr, mixin(arr)), Array.from(sArr));
}
let arrayOf = (BaseClass) => {
    let mods = [basicHooks, typedMixin, setMixin];
    ...
    // let bHooks = basicHooks(BaseClass);
    // let tHooks = typedMixin(BaseClass);
    // let sHooks = setMixin(BaseClass);
    let addMixin = (mod) => mods.push(mod);
    // addMixin(basicHooks);
    // addMixin(typedMixin);
    // addMixin(setMixin);
    let compVal = (sArr) => composeMixins(BaseClass)(...mods)(sArr);
    let construct3 = (sArr = []) => Object.assign(compVal(sArr));
    ...
    // let construct2 = (sArr = []) => Object.assign(Array.from(sArr), bHooks(sArr), tHooks(sArr), sHooks(sArr));
    // let instance2 = (sArr = []) => {
    //     let retVal = construct3(sArr);
    //     return retVal;
    // }
    return {
        instance: construct3,
