...
let hooksFromBase = (BaseClass) => (inputArr) => {
    ...
    let methods = {
        ...
        removeElement: (argObj) => {
            ...
            console.log("presplice", sArr.length);
            let retVal = eqIdx > -1 && methods.splice(eqIdx, 1);
            sArr = instanceFrom(sArr);
            console.log("postsplice", sArr.length);
            return retVal;
        },
        reassign: () => instanceFrom(sArr),

        // intersects: (altArray) => instanceFrom(sArr.some(currEl => altArray.contains(currEl) === true))
        // intersection: (altArray) => instanceFrom(sArr.filter(currEl => altArray.contains(currEl) === true))
        // hasDistinctElements: (altArray) => instanceFrom(sArr.some(myObj => !altArray.contains(myObj)))
