...
let dfs = ({ edges }) => (iNode) => {
	...
	let dVisit = (path) => {
		...
		let nextNabes = new Map(spreadEntries(neighbors({ edges })(pred))
			.filter(x_pathHasEntry(path)));
...
let unvisitedNeighbors = ({ edges }) => (path) => (node) => {
	return spreadKeys(neighbors({ edges })(node)).filter(x_pathHasNode(path));
};

let unvisitedEntries = ({ edges }) => (path) => (node) => {
	return spreadEntries(neighbors({ edges })(node)).filter(x_pathHasEntry(path));
};
