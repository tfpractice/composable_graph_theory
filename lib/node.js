...
let stringable = () => ({ label }) => ({ toString: () => label });
// let equalAccessor = ({ label: hostL = '' }) => ({ label: argL = '' }) =>
// hostL === argL;

// methodMakers
let typeMaker = typify.stateOps(typeAccessor);
let dataMaker = datafy(dataAccessor);
let labelMaker = labelize.stateOps(labelAccessor);

let equalAccessor = (state) => labelMaker(state).sameLabel;
let eqMaker = equalize.stateOps(equalAccessor);

let Node = factory(typeMaker, dataMaker, labelMaker, eqMaker, stringable());
