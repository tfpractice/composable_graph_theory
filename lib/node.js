...
// let equalAccessor = (state) => (arg) => state.label === arg.label();
// let equalAccessor = (state) => argObj.label();

// state_instance operators
let compareLabels = (state) => (argNode) =>
	labelAccessor(state) === getLabel(argNode);
...
let NodeFactory = factory(typeMaker, dataMaker, labelMaker, eqMaker);
