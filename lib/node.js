...
// state accessors
let label_accessor = (state) =>
    (state.label);
let equal_accessor = (argObj) =>
    argObj.label();
let data_accessor = (state) =>
    state.data;
let type_accessor = (state) =>
    'Node';
...
let getType = (node) =>
    node.type();
let areEquivalent = (argNode) =>
    (srcNode) =>
    srcNode.isEquivalent(argNode);


let nodePipeline = (state, nodeObject = {}) =>
    pipeline(mixin(nodeObject)((type_maker)(state)), modify(label_maker)(state), modify(data_maker)(state), modify(compare_to_host)(nodeObject))
