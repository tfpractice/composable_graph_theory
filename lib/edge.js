...
let equalityChecker = (state) => ({
    isEquivalent: (argEdge) => compareNodes(state).hasSameNodes(
        argEdge)
})
let hasNode = (state) => ({
...
let neighborArray = (state) => ({
    nabeArray: (xNode) => NodeArray.instance(state.nodes).excludeElement(
        xNode)
});
...
let containsNodes = (...nodes) => (edge) => nodes.map(containsNode).every(
    unaryCall(edge))
let getNeighborArray = (nabe) => (edge) => edge.getNeighbor(nabe)
let hasSameNodes = (argEdge) => (hostEdge) => hostEdge.nodes().isEquivalent(
    argEdge.nodes());
let isEquivalent = (argEdge) => (hostEdge) => hasSameNodes(hostEdge)(
    argEdge);
...
let stateFuncs = [labelMaker, weightMaker, typeMaker, nodeMaker,
    ...
    nabeMaker, compareLabel, compareNodes, hasNode
];
