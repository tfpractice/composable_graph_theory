...
let lFunc = (state) => state.nodes.reduce((str, n, id, a) =>
...
let hasNode = (state) =>
    ({
        ...
        // containsNode: (nodeArg) => NodeArray.isPresent(nodeArg)(NodeArray.instance(state.nodes))
    });
let nabeMaker = (state) =>
    ({
        getNeighbor: (nodeArg) =>
...
let containsNode = (node) =>
    (edge) => edge.containsNode(node);
let containsNodes = (...nodes) =>
    (edge) => nodes.map(containsNode).every(unaryCall(edge))
let getNeighborArray = (nabe) =>
    (edge) => edge.getNeighbor(nabe)
let hasSameNodes = (argEdge) =>
    (hostEdge) => hostEdge.nodes().isEquivalent(argEdge.nodes());
let isEquivalent = (argEdge) =>
    (hostEdge) => hasSameNodes(hostEdge)(argEdge);
let hasSameWeight = (argEdge) =>
    (hostEdge) => host.weight() === argEdge.weight();
let stateFuncs = [labelMaker, weightMaker, typeMaker, nodeMaker, neighborArray, compareWeight, equalityChecker, stringMaker, nabeMaker, compareLabel, compareNodes, hasNode];
