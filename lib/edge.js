...
let stateFuncs = [lab_maker, weightAx, nodify, neighborArray, compareWeight, equalityChecker, toString, nabeMaker, compareLabel, compareNodes, hasNode];
let Edge = (n0, n1, w = 0) => {
    let state = {
        nodes: [n0, n1],
        weight: w
    ...
    console.log(mixObj);
    // return Object.assign({}, lab_maker(state), neighborArray(state), compareWeight(state), equalityChecker(state), toString(state), nabeMaker(state), weightAx(state), compareLabel(state), compareNodes(state), hasNode(state), nodify(state));
    // return {
