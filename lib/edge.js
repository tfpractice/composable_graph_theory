...
let containsNodes = (...nodes) => (edge) => nodes.map(containsNode).every(unaryCall(edge))
let getNeighborArray = (nabe) => (edge) => edge.getNeighbor(nabe)
let hasSameNodes = (argEdge) => (hostEdge) => hostEdge.nodes().isEquivalent(argEdge.nodes());
let isEquivalent = (argEdge) => (hostEdge) => hasSameNodes(hostEdge)(argEdge);
let hasSameWeight = (argEdge) => (hostEdge) => host.weight() === argEdge.weight();
