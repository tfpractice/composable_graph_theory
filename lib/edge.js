 ...
 // let cFunc = (context) => (elem) => Node.areEquivalent(elem)(context)
 let checkForNode = (elem) => (edge) => NodeArray.isPresent(elem)(getNodes(
     edge));
 let evalNodes = (queryEdge) => (contextEdge) => (getNodes(queryEdge))
     .isEquivalent(getNodes(contextEdge))
 ...
 let nabeMaker = (state) => ({
     getNeighbor: (nodeArg) => NodeArray.spawn(state.nodes)
         .find(n => !n.isEquivalent(nodeArg))
 });
 let neighborArray = (state) => ({
     nabeArray: (xNode) => NodeArray.spawn(state.nodes)
