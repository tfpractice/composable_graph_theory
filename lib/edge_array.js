...
let edgesByArray = (eArr) => (...nArr) =>
    nArr.reduce((eSet, nNode) =>
        eSet.unionize(edgesWithNode(eArr)(nNode)), EdgeArray.instance(
            []));
...
let getNodes = (eArr) =>
    eArr.map(Edge.getNodes)
    .reduce((pArr, nArr) => nArr.unionize(pArr), []);

let getNeighbors = (eArr) => (nArg) =>
    edgesWithNode(eArr)(nArg)
    .map(e => e.nabeArray(nArg))
    .reduce((pArr, nArr) =>
        nArr.unionize(pArr), []);

let EdgeArray = arrayOf(setMixin(Edge.isEquivalent), typeMixin(() =>
    "EdgeArray"));
