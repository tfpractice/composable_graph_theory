	...
	describe('methods', () => {
		...
		describe('sameLabel', () => {
			it(
				...
				() => {
					expect(sLabel0.sameLabel(myLabel(state0))).toBeTrue();
					expect(sLabel0.sameLabel(myLabel(state1))).toBeFalse();
				});
		});
	});
	describe('stateOps ', () => {
		...
		describe('when when passed a stateObject', () => {
			...
			describe('label', () => {
				it('retrieves the label attribute ', () => {
					expect(stateOps(state0).label()).toBe('state0');
				});
			});
			describe('sameLabel', () => {
				it('it compares the lFunc on both state objects', () => {
					let s0 = stateOps(state0);
