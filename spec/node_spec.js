fdescribe('Node', function () {
  ...
  beforeAll(function () {
    ...
    eqFunc = state => arg => (labelize(lFunc)(state).sameLabel(
      arg));
    nType = typify(tFunc);
    nData = datafy(dFunc);
    nEq = equalize(eqFunc);
  ...
  describe('when given a state obj{label, data} ', () => {
    ...
    describe('#isEquivalent', () => {
      it(
        'returns true if the two objects share label', () => {
    ...
    describe('operators', () => {
      describe('getLabel(node)', () => {
        it('returns the nodes label', () => {
          expect(Node.getLabel(n2)).toEqual(
            n2.label());
        });
      });
      ...
      describe('isEquivalent(argNode)(srcNode)', () => {
        it(
          'returns a boolean based on node equality', () => {
