describe('arrayOf', () => {
    ...
    describe('when given a baseType', () => {
        ...
        describe('when given a set of mixins', () => {
            ...
            describe('instance', () => {
                it('returns an array with mixed in methods', function() {
                ...
                describe('intersects ', () => {
                    it('returns a boolean based on the presence of shared elements', function() {
                        expect(myArray.intersects(altArray)).toBeTrue();
                    });
                });
                describe('intersection ', () => {
                    it('returns an array of the shared elements', function() {
                        let shared = myArray.intersection(altArray)
                        expect(shared).toBeArray();
                        expect(shared.isSubset(myArray)).toBeTrue();
                    });
                });

                describe('difference ', () => {
                    it('returns an array of elemnets in the current array absent in the altArray', function() {
                        expect(altArray.difference(myArray)).toBeArray();
                        expect(myArray.difference(altArray)).toContain(n0);
                    });
                    it('returns an array with the same methods as the original', function() {
                        expect(myArray.difference(altArray).contains(n0)).toBeTrue();
                    });
                });
                ...
                describe('removeElement', function() {
                    it('calls the instanceFunc on the return val from slice', function() {
                        expect(myArray.removeElement(n1)).toBeArray();
                    });
                });
            });
        });
